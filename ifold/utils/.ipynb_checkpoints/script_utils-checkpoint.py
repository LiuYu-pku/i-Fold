...
from ifold.np import residue_constants, protein
from ifold.np.relax import relax
...
def count_models_to_evaluate(ifold_checkpoint_path, jax_param_path):
    model_count = 0
    if ifold_checkpoint_path:
        model_count += len(ifold_checkpoint_path.split(","))
    if jax_param_path:
...
def load_models_from_command_line(config, model_device, ifold_checkpoint_path, jax_param_path, output_dir):
    # Create the output directory

    multiple_model_mode = count_models_to_evaluate(ifold_checkpoint_path, jax_param_path) > 1
    if multiple_model_mode:
    ...
    if ifold_checkpoint_path:
        for path in ifold_checkpoint_path.split(","):
            model = AlphaFold(config)
            model = model.eval()
            ...
            if os.path.isdir(path):
                ...
                import_ifold_weights_(model=model, state_dict=d["ema"]["params"])
            else:
                ckpt_path = path
                d = torch.load(ckpt_path)
                ...
                if "ema" in d:
                    ...
                    d = d["ema"]["params"]
                import_ifold_weights_(model=model, state_dict=d)
    ...
    if not jax_param_path and not ifold_checkpoint_path:
        raise ValueError(
            "At least one of jax_param_path or ifold_checkpoint_path must "
            "be specified."
        )
