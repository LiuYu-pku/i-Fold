...
from ifold.np import residue_constants
from ifold.utils.rigid_utils import Rotation, Rigid
from ifold.utils.geometry.vector import Vec3Array, euclidean_distance
from ifold.utils.all_atom_multimer import get_rc_tensor
from ifold.utils.tensor_utils import (
    tree_map,
    masked_mean,
    permute_final_dims,
)
import logging
from ifold.utils.tensor_utils import tensor_tree_map

import esm
from esm import Alphabet, FastaBatchedDataset, ProteinBertModel, pretrained, MSATransformer

Esm_Model, Esm_alphabet = esm.pretrained.load_model_and_alphabet('/home2/kangboming/kangboming/workspace/PIC_revise/model/ESM/esm2_t33_650M_UR50D.pt')
...
def id2aa(id_lst, restypes = ["A","R","N","D","C","Q","E","G","H","I","L","K","M","F","P","S","T","W","Y","V"]):
    seq = ''
    for idx in id_lst:
        seq = seq + restypes[idx]
    return seq

def calculate_entropy_wt(batch, esm_model, esm_batch_converter):
    aa_type_idx = batch['aatype']
    device = batch["aatype"].device
    seq_lst = list(map(id2aa, batch['aatype']))
    data = []
    for item in seq_lst:
        data.append(("_", item))
    ...
    with torch.no_grad():
        logits = torch.softmax(esm_model(batch_tokens, repr_layers=[33], return_contacts=False)["logits"][:,1:-1,4:24], dim=-1)
    prob_matrix = torch.clip(logits, 1e-10, 1.0)
    entropy = -torch.sum(prob_matrix * torch.log2(prob_matrix), axis=-1)
    entropy_wt = 1 + 10/(entropy+1)
    # entropy_wt = 1 + 20/(entropy+1)
    # entropy_wt = (1 + 5/(entropy+1)) / (1 + 5/5.3)
    entropy_wt = entropy_wt.unsqueeze(0).repeat(8,1,1)
    return entropy_wt
...
def compute_fape(
    ...
    # print('error_dist_shape:', error_dist.shape)
    # print('use_esm_wt2:', use_esm_wt)
    ...
    if pair_mask is not None:
        ...
        # print('normed_error1:', normed_error, normed_error.shape)
        ...
        if (use_esm_wt) & (normed_error.shape[-1] == 256):
            # print('Entropy_WT:', Entropy_WT.shape)
            normed_error = normed_error * Entropy_WT
        # print('normed_error2:', normed_error, normed_error.shape)
        normed_error = torch.sum(normed_error, dim=-1)
...
def backbone_loss(
    ...
    **kwargs,
    ...
    fape_loss = compute_fape(
        ...
        eps=eps,
        use_esm_wt = use_esm_wt
    )
    if use_clamped_fape is not None:
        # print('use_clamped_fape:', use_clamped_fape)
    ...
    # print('backbone_fape_loss:', fape_loss)
...
def fape_loss(
    ...
    # print(config.use_esm_wt)
    if config.use_esm_wt:
        global Entropy_WT, Esm_Model, Esm_Batch_Converter
        Entropy_WT = calculate_entropy_wt(batch, Esm_Model, Esm_Batch_Converter)
